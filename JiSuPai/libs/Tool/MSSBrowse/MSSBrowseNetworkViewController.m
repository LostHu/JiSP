...
#define Window_SCREEN_WIDTH ([UIScreen mainScreen].bounds.size.width)
#define Window_SCREEN_HEIGHT ([UIScreen mainScreen].bounds.size.height)
...
- (void)loadBrowseImageWithBrowseItem:(MSSBrowseModel *)browseItem Cell:(MSSBrowseCollectionViewCell *)cell bigImageRect:(CGRect)bigImageRect
{
    ...
    // 判断大图是否存在
    //    if([[SDImageCache sharedImageCache]diskImageExistsWithKey:browseItem.bigImageUrl]) // sdimage 缓存
    if([[YYImageCache sharedCache] containsImageForKey:browseItem.bigImageUrl])
    {
        // 显示大图
...
//    imageView.image = [[SDImageCache sharedImageCache]imageFromDiskCacheForKey:browseItem.bigImageUrl]; // sdimage 缓存
    ...
    if(self.isFirstOpen)
    {
        ...
        if (browseItem.smallImageView) {
            ...
            imageView.frame = CGRectMake(Window_SCREEN_WIDTH/2 -20, Window_SCREEN_HEIGHT/2, 40, 40);
        }
...
- (void)loadBigImageWithBrowseItem:(MSSBrowseModel *)browseItem cell:(MSSBrowseCollectionViewCell *)cell rect:(CGRect)rect
{
    ...
    @weakify(imageView)
    [imageView setImageWithURL:[NSURL URLWithString:[NSString trimWhitespaceAndNewline:browseItem.bigImageUrl]]
                   placeholder:browseItem.smallImageView.image
